...
<tr class = "book-info-panel-row @if($rent->rent_state == "Zwrócono")returned @endif">
     <td class="book-info-panel-cell">{{DB::table('books')->where('id_book','=',$rent->book_id)->value('title')}}</td>
     <td class="book-info-panel-cell">{{DB::table('users')->where('id_user','=',$rent->user_id)->value('login') }}</td>
     <td class="book-info-panel-cell">{{DB::table('users')->where('id_user','=',$rent->user_id)->value('email') }}</td>
     ...
     <form method="POST" action="/blockuser">
        ...
        <button type="submit" name="function" class="block" value="block"><i class="fa-solid fa-ban"></i></button>
    ...
    <td class="book-info-panel-cell">
        ...
        <form method="POST" action="/givebook">
            ...
            <input type="hidden" name="id_rent" value="{{ $rent->id_rent }}">
            <input type="hidden" name="userId" value="{{ $rent->user_id }}">
            <button type="submit" class="give"><i class="fa-solid fa-hand-holding-hand"></i></button>
